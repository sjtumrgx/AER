...
try:
    ...
    def quat_rotate_inverse(q, v):
        ...
        return quat_mul(quat_mul(q_inv, torch.cat((v, zeros), dim=-1)), q)[..., :3]

    def quat_from_angle_axis(angle, axis):
        axis = axis / torch.clamp(torch.norm(axis), min=1e-8)
        half = angle * 0.5
        xyz = axis.view(1, 3) * torch.sin(half).view(-1, 1)
...
class CoRLRewards:
    ...
    def _reward_load_normalized_transport(self):
        """Payload-aware load-normalized transport cost.

        Computes CoT-like mechanical transport cost:
            E / ((m_robot + m_payload) * g * d)
        over one policy step, where E = sum_i |tau_i * qdot_i| * dt and
        d = ||v_xy|| * dt. The cost is multiplied by alpha_E according to the
        selected baseline:
        - none: alpha_E = 0
        - fixed: alpha_E = cfg.rewards.fixed_energy_alpha
        - adaptive: alpha_E = env.policy_energy_alpha, generated by the policy
        """
        mode = getattr(self.env.cfg.rewards, "energy_regularization_mode", "none")
        mechanical_power = torch.sum(torch.abs(self.env.dof_vel * self.env.torques), dim=1)
        speed = torch.norm(self.env.base_lin_vel[:, :2], dim=1)
        speed_floor = getattr(self.env.cfg.rewards, "transport_speed_floor", 0.05)
        speed = torch.clamp(speed, min=speed_floor)
        robot_payload_mass = torch.clamp(self.env.default_body_mass + self.env.payloads, min=1e-3)
        transport_cost = mechanical_power / (robot_payload_mass * 9.81 * speed)
        transport_cost = torch.clamp(transport_cost, max=getattr(self.env.cfg.rewards, "transport_cost_clip", 100.0))

        if mode == "none":
            alpha = torch.zeros_like(transport_cost)
        elif mode == "fixed":
            alpha = torch.ones_like(transport_cost) * float(getattr(self.env.cfg.rewards, "fixed_energy_alpha", 1.0))
        elif mode == "adaptive":
            if hasattr(self.env, "policy_energy_alpha"):
                alpha = self.env.policy_energy_alpha.view(-1).to(transport_cost.device)
            else:
                alpha = torch.ones_like(transport_cost) * float(getattr(self.env.cfg.rewards, "fixed_energy_alpha", 1.0))
        else:
            raise ValueError(f"Unsupported energy_regularization_mode: {mode}")
        return alpha * transport_cost
